...
exports.submitContactForm = (0, https_1.onCall)({
    ...
    secrets: [sendEmail_1.RESEND_API_KEY],
}, async (request) => {
    ...
    // system_settings/smtp foi descontinuado (migração para RESEND unificado).
    // Mantém-se apenas a leitura do destinatário de notificação em system_settings/contact.
    ...
    let sendResult;
    try {
        sendResult = await (0, sendEmail_1.sendEmail)({
            to: [toEmail],
            replyTo: email,
            subject,
            text,
        ...
        firebase_functions_1.logger.error('submitContactForm resend error', { error: String(err) });
